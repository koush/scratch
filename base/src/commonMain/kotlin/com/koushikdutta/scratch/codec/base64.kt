...
private fun encodeBase64(byteArray: ByteArray): ByteArray {
    val table = (CharRange('A', 'Z') + CharRange('a', 'z') + CharRange('0', '9') + '+' + '/').toCharArray()
    val output = ByteBufferList()
    var padding = 0
    var position = 0
...
private fun decodeBase64(byteArray: ByteArray): ByteArray {
    ...
    while (position < byteArray.size) {
        var b: Int
        if (table[byteArray[position].toInt()] != -1) {
            b = table[byteArray[position].toInt()] and 0xFF shl 18
        } else {
            position++
            continue
        }
        var count = 0
        if (position + 1 < byteArray.size && table[byteArray[position + 1].toInt()] != -1) {
            b = b or (table[byteArray[position + 1].toInt()] and 0xFF shl 12)
            count++
        }
        if (position + 2 < byteArray.size && table[byteArray[position + 2].toInt()] != -1) {
            b = b or (table[byteArray[position + 2].toInt()] and 0xFF shl 6)
            count++
        }
        if (position + 3 < byteArray.size && table[byteArray[position + 3].toInt()] != -1) {
            b = b or (table[byteArray[position + 3].toInt()] and 0xFF)
            count++
        }
        while (count > 0) {
...
fun EncodeExtensions<ByteArray>.base64(): String {
    return encodeBase64(value).decodeToString()
}

fun DecodeExtensions<String>.base64(): ByteArray {
    return decodeBase64(value.encodeToByteArray())
}
