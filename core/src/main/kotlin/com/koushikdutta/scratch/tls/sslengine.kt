...
typealias DefaultHostnameVerifier = OkHostnameVerifier
...
fun SSLEngine.runHandshakeTask() {
    delegatedTask?.run()
}

fun SSLEngine.checkHandshakeStatus(): SSLEngineHandshakeStatus {
    return handshakeStatus.convert()
}

// extension methods for unwrap with Buffers. manages overflows and allocations.
fun SSLEngine.unwrap(src: ByteBufferList, dst: WritableBuffers, tracker: AllocationTracker): SSLEngineResult {
...
fun SSLEngine.wrap(src: ByteBufferList, dst: WritableBuffers, tracker: AllocationTracker): SSLEngineResult {
    tracker.finishTracking()
    while (true) {
