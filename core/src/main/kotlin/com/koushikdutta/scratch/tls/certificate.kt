...
typealias RSAPrivateKey = RSAPrivateKey
typealias X509Certificate = X509Certificate
...
fun createSelfSignedCertificate(subjectName: String): Pair<RSAPrivateKey, X509Certificate> {
    val keyGen = KeyPairGenerator.getInstance("RSA")
    keyGen.initialize(2048)
    val pair = keyGen.generateKeyPair()
    val cert = createSelfSignedCertificate(pair, subjectName)
...
fun SSLContext.init(pk: RSAPrivateKey, certificate: X509Certificate): SSLContext {
...
fun SSLContext.init(certificate: X509Certificate): SSLContext {
    val ks = KeyStore.getInstance(KeyStore.getDefaultType())
    ks.load(null)
