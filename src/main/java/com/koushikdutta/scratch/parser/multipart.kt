...
private fun createPartBoundaries(boundary: String, parts: AsyncIterator<Part>): AsyncIterator<AsyncRead> {
    return asyncIterator {
        yield(ByteBufferList().putString("--").putString(boundary).reader())
        while (parts.hasNext()) {
            yield(ByteBufferList().putString("\r\n").reader())
            val part = parts.next()
            yield(ByteBufferList().putString(part.headers.toHeaderString()).reader())
            yield(part.body)
            yield(ByteBufferList().putString("\r\n--").putString(boundary).reader())
        }
        yield(ByteBufferList().putString("--\r\n").reader())
    }
}
    ...
    companion object {
        ...
        */
        ...
        suspend fun parseMultipart(boundary: String, reader: AsyncReader): Multipart {
            ...
            val boundaryBreak = "\r\n--$boundary"
            val boundaryBreakBytes = boundaryBreak.toByteArray()
            val boundaryEnd = "--\r\n"
            ...
            return Multipart(asyncIterator {
                while (true) {
                    ...
                    val partRead: AsyncRead = read@{
                        ...
                        if (scan == false)
                            throw IOException("Multipart stream ended unexpectedly.")
