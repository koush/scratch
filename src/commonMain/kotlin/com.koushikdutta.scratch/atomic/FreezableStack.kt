...
class FreezableStackNode<T, V>(internal val previous: FreezableStackNode<T, V>?, val value: T?, internal val frozen: Boolean, val size: Int, val accumulate: V)
class FreezableStack<T, V>(private val defaultAccumulate: V, private val accumulate: (accumulated: V, value: T) -> V): Freezable {
    private val atomicReference = AtomicReference<FreezableStackNode<T, V>?>(null)
    val accumulated: V
    ...
    fun push(value: T): FreezableStackNode<T, V> {
        while (true) {
            ...
            if (top?.frozen == true)
                return top
            val node = FreezableStackNode(top, value, false, (top?.size ?: -1) + 1, accumulate(top?.accumulate ?: defaultAccumulate, value))
    ...
    fun freeze(): Boolean {
        while (true) {
            ...
            val newTop = if (top == null)
                FreezableStackNode<T, V>(top, null, true, 0, defaultAccumulate)
            else
                FreezableStackNode(top, null, true, 0, defaultAccumulate)
        ...
        while (true) {
            ...
            if (top.frozen) {
                ...
                if (underFrozenTop == null)
                    return top
                val newTop = FreezableStackNode(underFrozenTop.previous, null, true, 0, defaultAccumulate)
                if (atomicReference.compareAndSet(top, newTop))
    ...
    fun <R> clear(initialValue: R, accumulate: (collector: R, value: T) -> R): R {
        while (true) {
            ...
            if (top.frozen) {
                val underFrozenTop = top.previous
                val newTop = FreezableStackNode<T, V>(null, null, true, 0, defaultAccumulate)
