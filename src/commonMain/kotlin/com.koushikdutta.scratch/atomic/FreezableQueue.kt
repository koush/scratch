...
class FreezableQueueNode<T>(val value: T?, val frozen: Boolean) {
    internal val next = AtomicReference<FreezableQueueNode<T>?>(null)
}
...
class FreezableQueue<T>: Freezable {
    private val tail = AtomicReference<FreezableQueueNode<T>?>(null)
    private val head = AtomicReference<FreezableQueueNode<T>?>(null)
